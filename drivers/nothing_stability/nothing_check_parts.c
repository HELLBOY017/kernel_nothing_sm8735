...
static int entry_handler(struct kprobe *ri, struct pt_regs *regs) {
	...
	if(!dm_verity->data_dev)
		goto OUT;
	if(!dm_verity->data_dev->bdev)
		goto OUT;
	if(!dm_verity->data_dev->bdev->bd_disk)
		goto OUT;
	if(!dm_verity->data_dev->bdev->bd_disk->private_data)
		goto OUT;

	md = (struct mapped_device *)dm_verity->data_dev->bdev->bd_disk->private_data;
	if(!md)
		goto OUT;
